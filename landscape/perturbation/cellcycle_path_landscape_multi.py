...
dyn.vf.topography(adata, n=200, basis='umap_perturbation');

VecFld_perturbation = adata.uns['VecFld_umap_perturbation']

def vector_field_function(x, VecFld=VecFld_perturbation, dim=None, kernel="full", X_ctrl_ind=None):
...
VecFnc_perturbation = vector_field_function
...
def path_function(D, dt, i):    
    ...
    for n_steps in np.arange(1, numTimeSteps):
        # update dxdt, dydt
        dxdt, dydt = VecFnc_perturbation([x_p, y_p])
